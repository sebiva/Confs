...
git_prompt=" %F{$cred}(%F{$cred}${git_branch}${git_dirty}%F{$cred})"
else
unset git_prompt
fi
...
prompt_is_ssh='%F{$cbrightblue}[%F{$cred}SSH%F{$cbrightblue}] '
elif over_ssh; then
prompt_is_ssh='%F{$cbrightblue}[%F{$cred}SSH%F{$cbrightblue}] '
else
unset prompt_is_ssh
...
case $USER in
root)
PROMPT='%F{$cred}%n:%k%(?..%F{$cyellow}[%F{$corange}%?%F{$cblue}] )${prompt_is_ssh}%F{$corange}%~${git_prompt}%F{$cblue} %b%f%k
%F{$cred} %# %f'
;;
*)

PROMPT='%F{$cdarkblue}%n%f:%k%(?..%F{$cyellow}[%F{$cblack}%?%F{$cblue}] )${prompt_is_ssh}%F{$cbrightblue}%~${git_prompt}%F{$cblue} %b%f%k
%F{$cred} %# %f'
;;
esac
